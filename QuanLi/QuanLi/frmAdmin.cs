...
namespace QuanLi
{
    public partial class frmAdmin : Form
    {
        ...
        public frmAdmin(string user, string pass, bool admin) : this()
        {
            username = user;
            password = pass;
            isAdmin = admin;
        ...
        {
            DataTable data = ClassAdmin.HandleAdmin.Instance.LoadDGV(username);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (isEdit)
            {
                if (ValidateChildren(ValidationConstraints.Enabled))
                {
                    ...
                    ClassAdmin.Admin admin = new ClassAdmin.Admin() { TenAD = tenAD, GioiTinh = gioiTinh, NgaySinh = ngaySinh, SDT = sdt, Email = email, DiaChi = diaChi, AccAdmin = username };
                    if (ClassAdmin.HandleAdmin.Instance.Edit(admin))
                    {
        ...
        private void txtTenAD_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                errTenAD.SetError(txtTenAD, "Tên người quản lí không được để trống");
            }
            else if (tenAD.Length < 2 || tenAD.Length > 20)
            {
                e.Cancel = true;
                errTenAD.SetError(txtTenAD, "Tên người quản lí không hợp lệ");
            }
            else
            {
                e.Cancel = false;
